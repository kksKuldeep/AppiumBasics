...
public class eCommerceTest_hybridAppFlow extends BaseClass{
	...
	public void VerifyProductsAndProceedToCheckout() throws InterruptedException
	{
		...
		Thread.sleep(5000);
		//Hybrid App Flow Started >> Google page
		//Context switch from App driver to Native driver
		Set<String>conexts=driver.getContextHandles();
		for(String conextName:conexts)
		{
			System.out.println(conextName +" this is webview");
		}
		
		driver.context("NATIVE_APP"); //chrome driver
		//driver.findElement(AppiumBy.id("recaptcha-anchor")).click();
		//enter value in Google browser search textbox
		driver.findElement(AppiumBy.xpath("//android.widget.EditText[@displayed='true']")).sendKeys("Kuldeep User");
		driver.findElement(AppiumBy.xpath("//android.widget.EditText[@displayed='true']")).sendKeys(Keys.ENTER);
		driver.pressKey(new KeyEvent(AndroidKey.BACK));
